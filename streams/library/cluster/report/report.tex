...
A StreamIt application is compiled into a set of threads where each 
threadÕs code is implemented using the C language. The existing static 
partitioner provides a mapping between the threads and a given number 
of cluster machines, which is stored in a cluster configuration file. 
Multiple threads can be mapped to a single machine.
...
a new file called a checkpoint. 
...
Our design begins with the static mapping of threads to nodes created 
by the StreamIt compiler. It collects statistics about the running 
program and periodically changes the mapping in order to improve 
balancing of CPU resources. The result is that the program is able to 
reconfigure itself for optimization and react to changing conditions, 
such as other programs running on the same cluster.
...
StreamIt compiler. This information includes the communication 
bandwidth between each pair of threads, and an estimate of the CPU 
resources required by each thread relative to other threads. This 
estimated work ratio, when combined with actual CPU usage measurements, 
allows us to estimate the actual CPU utilization of each thread.
...
partitionÕs work estimate becoming larger than a certain threshold. 
...
Every 1 to 5 seconds, the CCP sends an ALIVE\_COMMAND message to the 
all the connected nodes participating in the StreamIt computation. 
Each node sends its current CPU utilization along with its greatest 
checkpoint number to the CCP.

Whenever the CCP detects that a reconfiguration is necessary, it sends 
a STOP\_THREAD message to all the nodes and then sends a new mapping 
along with the checkpoint number from which computation should resume. 
