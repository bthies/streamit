...
// //to make sure that dynamic rates only occur at the endpoints of static sub
// graphs
...
// SIRFilterIter iter) {
// assert !shouldSeeNoMoreFilters : "Dynamic Rates in middle of static sub graph
// " + self;

// if (self.getPop().isDynamic()) {
// //convert any dynamic pop rate to zero!!
// self.setPop(new JIntLiteral(0));
// assert !visitedAFilter : "Dynamic Rates in the middle of static sub graph " +
// self;
// }
...
// while (current != null) {
// IterFactory.createFactory().createIter(current.getTopLevelSIR()).accept(new
