 ...
 */
public class StreamItAutoIndentStrategy extends DefaultAutoIndentStrategy {

    public StreamItAutoIndentStrategy() {
    }
    ...
    protected int findMatchingOpenBracket(IDocument document, int line, int end,
		...
		// negative, opening positive) until we find a line the brings the 
		// count to zero
		while (brackcount < 0) {
		    line--;
		    if (line < 0) {
    ...
    protected void smartIndentAfterNewLine(IDocument document, 
		...
		try {
		    ...
		    System.out.println(StreamItEditorMessages.
