...
import com.jgraph.plaf.basic.TransferHandler; // JAVA13: com.jgraph.plaf.basic.TransferHandler;
...
public class BasicGraphUI extends GraphUI implements Serializable {
	...
	public class GraphTransferHandler extends TransferHandler {
		...
		protected final Transferable createTransferable(JComponent c) {
			if (c instanceof JGraph) {
				...
				if (cells != null && cells.length > 0) {
					...
					out = cells;
					return create(graph, cells, cs, viewAttributes, bounds);
				}
			}
			return null;
		}
		...
		protected GraphTransferable create(
			...
			Map viewAttributes,
			Rectangle bounds) {
			return new GraphTransferable(cells, cs, viewAttributes, bounds);
		}
	...
	public void insertCells(
		...
		if (clone) {
			...
			cs = cs.clone(cellMap);
			insert = new Object[cells.length];
			for (int i = 0; i < cells.length; i++)
				insert[i] = cellMap.get(cells[i]);
			insert = graph.getDescendantList(insert);
			pm = ParentMap.create(graphModel, insert, false);
		} else
			insert = cells;
		graphLayoutCache.insert(insert, viewAttributeMap, cs, pm, null);
	}
