...
public class StreamlinedDuplicate {
    ...
    private HashMap<SIRFilter, Integer> compToComm;
    ...
    private void calculateModel(SIRStream str) {
        ...
        compToComm = new HashMap<SIRFilter, Integer>();
        ...
        for (int i = workList.size() - 1; i >= 0; i--) {
            ...
            int comp = work.getWork(workList.getFilter(i));
            totalWork += comp;
            filtersToSched.add(workList.getFilter(i));
            int comm = (workList.getFilter(i).getPopInt() + workList.getFilter(i).getPushInt()) * 
    ...
    public void doit(SIRStream str, RawChip chip) {
        ...
        int workScheduledSoFar = 0;
        int workOnTile[] = new int[chip.getTotalTiles()];
        ...
        for (int tile = 0; tile < chip.getTotalTiles(); tile++) {
            int minWork = (totalWork - workScheduledSoFar) / (chip.getTotalTiles() - tile);
            int maxWork = (int)(((double)minWork) * 1.10);
    ...
    private int[] fissAmount(SIRFilter filter, int totalWork, 
            int maxWork, int minWork) {
        int ratio[] = new int[2];
        ...
        int go = (totalWork - maxWork);
        System.out.println("   goWork: " + go);
        int stay = (work.getWork(filter) - go);
    ...
    private SIRFilter fissMe(LinkedList<SIRFilter> filters, int totalWork, 
            int maxWork, int minWork) {
