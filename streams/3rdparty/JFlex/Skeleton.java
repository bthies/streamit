...
public class Skeleton {
  ...
  public static String line[] = {
    ...
    NL+
    "  /**"+NL+
    "   * Gets the next input character."+NL+
    "   *"+NL+
    "   * @return      the next character of the input stream, EOF if the"+NL+
    "   *              end of the stream is reached."+NL+
    "   * @exception   IOException  if any I/O-Error occurs"+NL+
    "   */"+NL+
    "  private int yy_advance() throws java.io.IOException {"+NL+
    NL+
    "    /* standard case */"+NL+
    "    if (yy_currentPos < yy_endRead) return yy_buffer[yy_currentPos++];"+NL+
    NL+
    "    /* if the eof is reached, we don't need to work hard */ "+NL+
    "    if (yy_atEOF) return YYEOF;"+NL+
    NL+
    "    /* otherwise: need to refill the buffer */"+NL+
    NL+
    ...
    NL+
    "    if ( numRead == -1 ) return YYEOF;"+NL+
    NL+
    "    yy_endRead+= numRead;"+NL+
    NL+
    "    return yy_buffer[yy_currentPos++];"+NL+
    "  }"+NL+
    NL+
    NL+
    "  /**"+NL+
    ...
    "      yy_action = -1;"+NL+
    NL+
    "      yy_currentPos = yy_startRead = yy_markedPos;"+NL+
    NL
    ...
    "        while (true) {"+NL+
    NL+
    "          yy_input = yy_advance();"+NL+
    NL+
    "          if ( yy_input == YYEOF ) break yy_forAction;"+NL+
    NL
    ,  // line, col, char count, next transition, isFinal action
    "            yy_action = yy_state; "+NL+
    "            yy_markedPos = yy_currentPos; "+NL
    ,  // line count update
    "          }"+NL+
    NL+
    "        }"+NL+
  ...
  /**
   * The current part of the skeleton (a index of nextStop[])
   */
  private int pos;
