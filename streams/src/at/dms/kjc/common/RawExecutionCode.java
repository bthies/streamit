...
import at.dms.kjc.*;
import at.dms.kjc.raw.DirectCommunication;
import at.dms.kjc.raw.FlatIRToC;
import at.dms.kjc.raw.IMEMEstimation;
import at.dms.kjc.raw.Layout;
import at.dms.kjc.raw.RawBackend;
import at.dms.kjc.raw.SwitchCode;
    ...
    implements FlatVisitor, Constants
{
    ...
    private int getUpStreamItems(HashMap executionCounts, FlatNode node) 
    {
        ...
        //schedule
        int prevInitCount = RawUtil.getCountPrev(executionCounts, 
                                              previous, node);
    ...
    **/
    private int getPrevSplitterUpStreamItems(FlatNode prev, FlatNode node) 
    {
        ...
        while (current.contents instanceof SIRSplitter) {
            if (!(((SIRSplitter)current.contents).getType() == SIRSplitType.DUPLICATE))
                roundRobinMult *= RawUtil.getRRSplitterWeight(current, downstream);
            if (current.inputs < 1)
                return 0;
            downstream = current;
                                      ...
                                      LocalVariables localVariables)
    {
        ...
        if (!noBuffer(filter)) {
            ...
            if (isSimple(filter)) {
                ...
                if (KjcOptions.ratematch)
                    buffersize = 
                        RawUtil.nextPow2(Math.max((((Integer)RawBackend.steadyExecutionCounts.get(node)).intValue() - 1) * 
                                               filter.getPopInt() + filter.getPeekInt(), prepeek) + remaining);
                else
                    buffersize = RawUtil.nextPow2(maxpeek + remaining);
            }
        ...
        if (KjcOptions.ratematch && filter.getPushInt() > 0) {
            ...
            dims[0] = new JIntLiteral(steady * filter.getPushInt() * 
                                      RawUtil.getTypeSize(filter.getOutputType()));
                                            ...
                                            LocalVariables localVariables) 
    {
        ...
        if (filter.getPushInt() > 0) {
            ...
            pushExp.setTapeType(RawUtil.getBaseType(filter.getOutputType()));
