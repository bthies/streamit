...
public class ClusterExecutionCode extends at.dms.util.Utils implements
        FlatVisitor, Constants {
    ...
    public void visitNode(FlatNode node) {
        if (node.isFilter()) {
            ...
            /*
             * System.out.print("ClusterExecCode visiting: "+filter.getName()+ "
             * [FILTER]" );
             * 
             * if (isSimple(filter)) System.out.print("(simple) "); else if
             * (noBuffer(filter)) System.out.print("(no buffer) "); else { if
             * (remaining > 0) System.out.print("(remaining) "); if
            ...
            JMethodDeclaration clusterMainFunct = new JMethodDeclaration(null,
                    at.dms.kjc.Constants.ACC_PUBLIC, CStdType.Void, rawMain,
                    JFormalParameter.EMPTY, CClassType.EMPTY, block, null, null);
    ...
    private void createLocalVariables(FlatNode node, JBlock block,
        ...
        JVariableDefinition exeIndexVar = new JVariableDefinition(null, 0,
                CStdType.Integer, exeIndex, null);
        ...
        JVariableDefinition exeIndex1Var = new JVariableDefinition(null, 0,
                CStdType.Integer, exeIndex1, null);
        ...
        if (!noBuffer(filter)) {
            ...
            if (isSimple(filter)) {
                ...
                if (KjcOptions.ratematch) {
                    ...
                    /* int i = */((Integer) ClusterBackend.steadyExecutionCounts
                            .get(node)).intValue();
                    ...
                    buffersize = Math.max(
                            (((Integer) ClusterBackend.steadyExecutionCounts
                                    .get(node)).intValue() - 1)
                ...
                JVariableDefinition simpleIndexVar = new JVariableDefinition(
                        null, 0, CStdType.Integer, simpleIndex,
                        new JIntLiteral(-1));
                ...
                // Section)
                if (KjcOptions.ratematch)
                    buffersize = Util.nextPow2(Math.max(
                            (((Integer) ClusterBackend.steadyExecutionCounts
                                    .get(node)).intValue() - 1)
                                    * filter.getPopInt() + filter.getPeekInt(),
                            prepeek)
            ...
            JVariableDefinition recvBufVar = new JVariableDefinition(null,
                    ...
                    new CArrayType(filter.getInputType(), 1,
        ...
        if (KjcOptions.ratematch && filter.getPushInt() > 0) {
            ...
            JVariableDefinition sendBufferIndexVar = new JVariableDefinition(
                    null, 0, CStdType.Integer, sendBufferIndex,
                    new JIntLiteral(-1));
            ...
            JVariableDefinition sendBufVar = new JVariableDefinition(null,
                    at.dms.kjc.Constants.ACC_FINAL, // ?????????
                    new CArrayType(filter.getOutputType(), 1, dims),
                    sendBuffer, null);
        ...
        if (filter.getInputType().isArrayType()
            ...
            if (filter.getOutputType().isArrayType())
                outputDim = ((CArrayType) filter.getOutputType())
                        .getArrayBound();
            ...
            for (int i = 0; i < maxDim; i++) {
                JVariableDefinition arrayIndexVar = new JVariableDefinition(
                        null, 0, CStdType.Integer, ARRAY_INDEX + i, null);
    ...
    private void clusterMainFunction(FlatNode node, JBlock statements,
        ...
        assert !(filter instanceof SIRPredefinedFilter)
                || (filter instanceof SIRIdentity) : "Found a "
                + filter.getName()
         ...
         * 
         * //create the params list, for some reason //calling toArray() on the
         * list breaks a later pass List paramList = filter.getParams();
         * JExpression[] paramArray; if (paramList == null || paramList.size() ==
         * 0) paramArray = new JExpression[0]; else paramArray =
         * (JExpression[])paramList.toArray(new JExpression[0]);
         * 
         * //add the call to the init function statements.addStatement (new
         * JExpressionStatement(null, new JMethodCallExpression (null, new
         * JThisExpression(null), filter.getInit().getName(), paramArray),
         * null));
         * 
         */
        ...
        if (ClusterBackend.FILTER_DEBUG_MODE) {
            statements.addStatement(new SIRPrintStatement(null,
                    new JStringLiteral(null, filter.getName()
                            + " Starting Steady-State"), null));
        }

        // add the call to the work function
        statements.addStatement(generateSteadyStateLoop(node, localVariables));
     ...
     * LocalVariables localVars) { SLIRReplacingVisitor convert;
     * 
     * if (simple) { convert = new ConvertCommunicationSimple(localVars); } else
     * convert = new ConvertCommunication(localVars);
     * 
     * JMethodDeclaration[] methods = filter.getMethods(); for (int i = 0; i <
     * methods.length; i++) { //iterate over the statements and call the
     * ConvertCommunication //class to convert peek, pop for (ListIterator it =
     * methods[i].getStatementIterator(); it.hasNext(); ){
    ...
    JStatement receiveCode(SIRFilter filter, CType type,
        ...
        JExpression exp = new JMethodCallExpression(null, new JThisExpression(
                null), receiveMethodName, bufferAccess);
    ...
    JStatement generateInitWorkLoop(SIRFilter filter,
        ...
        if (ClusterBackend.FILTER_DEBUG_MODE) {
            block.addStatement(new SIRPrintStatement(null, new JStringLiteral(
                    null, filter.getName() + " firing (init)."), null));
        }
    ...
    private static JStatement makeForLoop(JStatement body, JLocalVariable var,
        ...
        // debugging: print caller, and insert caller as comment on returned for
        // loop
        if (getForLoopCallers) {
            ...
            JavaStyleComment comment = new JavaStyleComment(caller, true,
                    false, false);
        ...
        return new JForStatement(null, init, cond, incr, body, comments);
    }
    ...
    JStatement generateSteadyStateLoop(FlatNode node,
        ...
        block.addStatement(new JExpressionStatement(null,
                new JMethodCallExpression(null, "check_status",
                        new JExpression[0]), null));

        block.addStatement(new JExpressionStatement(null,
                new JMethodCallExpression(null, "check_messages",
                        new JExpression[0]), null));

        block.addStatement(new JExpressionStatement(null,
                new JMethodCallExpression(null, "__update_pop_buf",
                        new JExpression[0]), null));
        ...
        if (ClusterBackend.FILTER_DEBUG_MODE) {
            block.addStatement(new SIRPrintStatement(null, new JStringLiteral(
                    null, filter.getName() + " firing."), null));
        }
        ...
        block.addStatement(new JExpressionStatement(null,
                new JMethodCallExpression(null, "send_credits",
                        new JExpression[0]), null));

        JVariableDefinition var_counter = new JVariableDefinition(null, 0,
                (CType) CStdType.Integer, "__counter_" + nodeID,
                new JIntLiteral(0));

        // block.addStatement();

        JVariableDefinition var_steady = new JVariableDefinition(null, 0,
                (CType) CStdType.Integer, "__steady_" + nodeID,
                new JIntLiteral(0));

        JVariableDefinition var_tmp = new JVariableDefinition(null, 0,
                (CType) CStdType.Integer, "__tmp_" + nodeID, new JIntLiteral(0));

        JVariableDefinition var_tmp2 = new JVariableDefinition(null, 0,
                (CType) CStdType.Integer, "__tmp2_" + nodeID,
                new JIntLiteral(0));

        JVariableDefinition var_freq = new JVariableDefinition(null, 0,
                (CType) CStdType.Integer, "__frequency_of_chkpts",
                new JIntLiteral(0));

        JVariableDefinition var2 = new JVariableDefinition(null, 0,
                (CType) CStdType.Integer, "__number_of_iterations_" + nodeID,
                new JIntLiteral(0));

        /*
         * JVariableDeclarationStatement var_st = new
         * JVariableDeclarationStatement( null, var, null);
         */

        // num iters = init + num_steady_iterations_specified_by_user * steady
        Integer initCounts = (Integer) ClusterBackend.initExecutionCounts
                .get(node);
        int init;
        if (initCounts == null) {
            init = 0;
        ...
        Integer count = (Integer) ClusterBackend.steadyExecutionCounts
                .get(node);
        if (count != null) {
            ...
            counts = -1;
            Utils.fail("don't know how many times to execute node "
                    + node.contents);
        }
        ...
        JExpression relation_if = new JRelationalExpression(null,
                Constants.OPE_LE,
                new JLocalVariableExpression(null, var_steady),
                new JIntLiteral(0));

        JExpressionStatement incr_steady = new JExpressionStatement(null,
                new JPostfixExpression(null, OPE_POSTINC,
                        new JLocalVariableExpression(null, var_steady)), null);

        JStatement init_stmt1 = new JExpressionStatement(null,
                (new JAssignmentExpression(null, new JLocalVariableExpression(
                        null, var_counter), new JIntLiteral(1))), null);

        JExpression relation1 = new JRelationalExpression(null,
                Constants.OPE_LE, new JLocalVariableExpression(null,
                        var_counter), new JIntLiteral(init));

        JStatement incr_expr1 = new JExpressionStatement(null,
                new JPostfixExpression(null, OPE_POSTINC,
                        new JLocalVariableExpression(null, var_counter)), null);

        JStatement init_stmt2 = new JExpressionStatement(null,
                (new JAssignmentExpression(null, new JLocalVariableExpression(
                        null, var_tmp2), new JIntLiteral(1))), null);

        JExpression relation2 = new JRelationalExpression(null,
                Constants.OPE_LE, new JLocalVariableExpression(null, var_tmp2),
                new JLocalVariableExpression(null, var2));

        JExpression _ll[] = new JExpression[2];

        _ll[0] = new JPostfixExpression(null, OPE_POSTINC,
                new JLocalVariableExpression(null, var_tmp2));

        _ll[1] = new JPostfixExpression(null, OPE_POSTINC,
                new JLocalVariableExpression(null, var_steady));
        ...
        JStatement init_stmt3 = new JExpressionStatement(null,
                (new JAssignmentExpression(null, new JLocalVariableExpression(
                        null, var_tmp), new JIntLiteral(1))), null);

        JExpression relation3 = new JRelationalExpression(null,
                Constants.OPE_LE, new JLocalVariableExpression(null, var_tmp),
                new JIntLiteral(counts));

        JExpression ll[] = new JExpression[2];

        ll[0] = new JPostfixExpression(null, OPE_POSTINC,
                new JLocalVariableExpression(null, var_tmp));

        ll[1] = new JPostfixExpression(null, OPE_POSTINC,
                new JLocalVariableExpression(null, var_counter));

        JStatement incr_expr3 = new JExpressionListStatement(null, ll, null);

        /*
         * 
         * JExpression numIters = new JAddExpression(null, new
         * JIntLiteral(init), new JMultExpression(null, new
         * JLocalVariableExpression(null, var2), new JIntLiteral(counts)));
         * 
         * JExpression relation = new JRelationalExpression(null,
         * Constants.OPE_LE, new JLocalVariableExpression(null, var), numIters);
         * 
         */

        JBlock for_block = new JBlock(null, new JStatement[0], null);

        for_block.addStatement(new JExpressionStatement(null,
                new JMethodCallExpression(null, "send_credits",
                        new JExpression[0]), null));

        for_block.addStatement(new JIfStatement(null, relation_if,

                new JForStatement(null, init_stmt1, relation1, incr_expr1,
                        block, null), new JEmptyStatement(null, null), 
                        new JavaStyleComment[] {
                        new JavaStyleComment("ClusterExecutionCode1", true,
                                false, false)}));
        ...
        sss[0] = new JForStatement(null, init_stmt3, relation3, incr_expr3,
                block, new JavaStyleComment[] {
                        new JavaStyleComment("ClusterExecutionCode2", true,
                                false, false)});

        JExpression s_params[] = new JExpression[3];

        s_params[0] = new JLocalVariableExpression(null,
                new JVariableDefinition(null, 0, (CType) CStdType.Integer,
                        "__thread_" + nodeID, new JIntLiteral(0)));
        s_params[1] = new JLocalVariableExpression(null,
                new JVariableDefinition(null, 0, (CType) CStdType.Integer,
                        "__steady_" + nodeID, new JIntLiteral(0)));
        s_params[2] = new JLocalVariableExpression(null,
                new JVariableDefinition(null, 0, (CType) CStdType.Integer,
                        "__write_thread__" + nodeID, new JIntLiteral(0)));

        JIfStatement nested_if_stmt = new JIfStatement(null,

                new JRelationalExpression(null, Constants.OPE_LE,
                        new JModuloExpression(null,
                                new JLocalVariableExpression(null, var_tmp2),
                                new JLocalVariableExpression(null, var_freq)),
                        new JIntLiteral(0)),

                new JExpressionStatement(null, new JMethodCallExpression(null,
                        "save_state::save_to_file", s_params), null),
        ...
        new JRelationalExpression(null, Constants.OPE_GT,
                new JLocalVariableExpression(null, var_freq),
                new JIntLiteral(0)),
        ...
        for_block.addStatement(new JForStatement(null, init_stmt2, relation2,
                incr_expr2,
                steady_state,
                new JavaStyleComment[] {
                        new JavaStyleComment("ClusterExecutionCode3", true,
                                false, false)
                }));
    ...
    class ConvertCommunicationSimple extends SLIRReplacingVisitor {
        ...
        public Object visitPopExpression(SIRPopExpression oldSelf,
            ...
            // debugging
            System.err.println("ConvertCOmmunicationSimple.visitPopExpression");
    ...
    class ConvertCommunication extends SLIRReplacingVisitor {
        ...
        public Object visitPopExpression(SIRPopExpression self, CType tapeType) {
            ...
            // debugging
            System.err.println("ConvertCommunication.visitPopExpression");
            assert self.getNumPop() == 1 : "need code here for multi-pops";
            // create the increment of the index var
            JPrefixExpression bufferIncrement = new JPrefixExpression(null,
                    OPE_PREINC, new JLocalVariableExpression(null,
    ...
    class RateMatchConvertPush extends SLIRReplacingVisitor {
        ...
        public Object visitPushExpression(SIRPushExpression self,
            ...
            JMethodCallExpression ratematchsend = new JMethodCallExpression(
                    null, new JThisExpression(null),
                    ClusterExecutionCode.rateMatchSendMethod, args);
    ...
    /*
     * public void visitNode(FlatNode node) { if (node.isFilter()){ SIRFilter
     * filter = (SIRFilter)node.contents; //Skip Identities now if(filter
     * instanceof SIRIdentity || filter instanceof SIRFileWriter || filter
     * instanceof SIRFileReader) return; if (!KjcOptions.decoupled)
     * calculateItems(filter); System.out.print("Generating Cluster Code: " +
     * node.contents.getName() + " ");
     * 
     * //attempt to generate direct communication code //(no buffer), if this
     * returns true it was sucessful //and the code was produced if (bottomPeek ==
     * 0 && remaining == 0 &&
     * DirectCommunication.doit((SIRFilter)node.contents)) {
     * System.out.println("(Direct Communication)"); return; }
     * 
     * //otherwise generate code the old way LocalVariables localVariables = new
     * LocalVariables();
     * 
     * JBlock block = new JBlock(null, new JStatement[0], null);
     * 
     * if (isSimple(filter)) System.out.print("(simple) "); else if
     * (noBuffer(filter)) System.out.print("(no buffer) "); else { if (remaining >
     * 0) System.out.print("(remaining) "); if (filter.getPeekInt() >
     * filter.getPopInt()) System.out.print("(peeking)");
     *  } System.out.println();
     * 
     * createLocalVariables(node, block, localVariables);
     ...
     * clusterMainFunction(node, block, localVariables);
     * 
     * //create the method and add it to the filter JMethodDeclaration
     * clusterMainFunct = new JMethodDeclaration(null,
     * at.dms.kjc.Constants.ACC_PUBLIC, CStdType.Void, rawMain,
     * JFormalParameter.EMPTY, CClassType.EMPTY, block, null, null);
     * filter.addMethod(clusterMainFunct); } }
     * 
     * //calcuate bottomPeek, initFire, remaining //see my thesis section 5.1.2
     * void calculateItems(SIRFilter filter) { int pop = filter.getPopInt(); int
     * peek = filter.getPeekInt();
     * 
     * //set up prePop, prePeek int prePop = 0; int prePeek = 0;
     * 
     * if (filter instanceof SIRTwoStageFilter) { prePop =
     * ((SIRTwoStageFilter)filter).getInitPop(); prePeek =
     * ((SIRTwoStageFilter)filter).getInitPeek(); }
     * 
     * //the number of times this filter fires in the initialization //schedule
     * initFire = 0;
     *  // initExec counts might be null if we're calling for work // estimation
     * before exec counts have been determined. if
     * (ClusterBackend.initExecutionCounts!=null) { Integer init =
     * (Integer)ClusterBackend.initExecutionCounts.
     * get(Layout.getNode(Layout.getTile(filter)));
     * 
     * if (init != null) initFire = init.intValue(); } else { // otherwise, we
     * should be doing this only for work // estimation--check that the filter
     * is the only thing in the graph assert filter.getParent()==null ||
     * filter.getParent().size()==1 && filter.getParent().getParent()==null:
     * "Found null pointer where unexpected."; }
     * 
     * //if this is not a twostage, fake it by adding to initFire, //so we
     * always think the preWork is called if (!(filter instanceof
     * SIRTwoStageFilter)) initFire++;
     * 
     * //the number of items produced by the upstream filter in //initialization
     * int upStreamItems = 0;
     * 
     * FlatNode node = Layout.getNode(Layout.getTile(filter)); FlatNode previous =
     * null;
     * 
     * if (node.inputs > 0) { previous = node.incoming[0]; //okay the number of
     * items produced by the upstream splitter may not //equal the number of
     * items that produced by the filter feeding the splitter //now, since I do
     * not map splitter, this discrepancy must be accounted for. //We do not
     * have an incoming buffer for ths splitter, so the data must //trickle down
     * to the filter(s) that the splitter feeds if (previous.contents instanceof
     * SIRSplitter) { upStreamItems = getPrevSplitterUpStreamItems(previous,
     * node); } else { //upstream not a splitter, just get the number of
     * executions upStreamItems =
     * getUpStreamItems(ClusterBackend.initExecutionCounts, node); } }
     * 
     * //see my thesis for an explanation of this calculation if (initFire - 1 >
     * 0) { bottomPeek = Math.max(0, peek - (prePeek - prePop)); } else
     * bottomPeek = 0;
     * 
     * remaining = upStreamItems - (prePeek + bottomPeek + Math.max((initFire -
     * 2), 0) * pop);
     * 
     * //System.out.println(remaining); }
     *  // node is not directly connected upstream to a splitter, this function //
     * will calculate the number of items send to node private int
     * getUpStreamItems(HashMap executionCounts, FlatNode node) { //if the node
     * has not incoming channels then just return 0 if (node.inputs < 1) return
     * 0;
     * 
     ...
     * int prevPush = 0;
     * 
     * //get the number of times the previous node executes in the //schedule
     * int prevInitCount = Util.getCountPrev(executionCounts, previous, node);
     * 
     * //get the push rate for the previous if (prevInitCount > 0) { if
     * (previous.contents instanceof SIRSplitter || previous.contents instanceof
     * SIRJoiner) { prevPush = 1; } else prevPush =
     * ((SIRFilter)previous.contents).getPushInt(); }
     * 
     * //push * executions int upStreamItems = (prevInitCount * prevPush);
     * 
     * //System.out.println("previous: " + previous.getName());
     * //System.out.println("prev Push: " + prevPush + " prev init: " +
     * prevInitCount);
     * 
     * //if the previous node is a two stage filter then count its initWork //in
     * the initialItemsTo Receive if (previous != null && previous.contents
     * instanceof SIRTwoStageFilter) { upStreamItems -=
     * ((SIRTwoStageFilter)previous.contents).getPushInt(); upStreamItems +=
     * ((SIRTwoStageFilter)previous.contents).getInitPush(); }
     * 
     * return upStreamItems; }
     * 
     *  // // If the filter's upstream neighbor is a splitter we may have a
     * problem // where the filter feeding the splitter produces more data than
     * the splitter // passes on in the init stage. So we need to recognize this
     * and forward the data // on to the filter(s) that the splitter feeds.
     * Remember, we do not map splitters // so the data on the splitters input
     * tape after the init stage is over must go somewhere.
     * 
     * private int getPrevSplitterUpStreamItems(FlatNode prev, FlatNode node) {
     * double roundRobinMult = 1.0;
     * 
     * //there is nothing feeding this splitter, so just return 0 if
     * (prev.inputs < 1) return 0;
     * 
     * //follow the channels backward until we get to a filter or joiner,
     * //remembering the weights on the rr splitters that connect //the filter
     * (joiner) to node, so we know the number of items passed to <node>
     * FlatNode current = prev; FlatNode downstream = node; while
     * (current.contents instanceof SIRSplitter) { if
     * (!(((SIRSplitter)current.contents).getType() == SIRSplitType.DUPLICATE))
     * roundRobinMult *= Util.getRRSplitterWeight(current, downstream); if
     * (current.inputs < 1) return 0; downstream = current; current =
     * current.incoming[0]; }
     * 
     * //now current must be a joiner or filter //get the number of item current
     * produces int currentUpStreamItems =
     * getUpStreamItems(ClusterBackend.initExecutionCounts, current.edges[0]);
     * 
     * //return the number of items passed from current to node thru the
     * splitters //(some may be roundrobin so we must use the weight multiplier.
