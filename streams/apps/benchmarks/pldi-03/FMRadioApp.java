...
/**
 * This program is a snapshot of streams/apps/benchmarks/fm/streamit/LinkedFMTest.java
 * that was co-opted for the PLDI'03 paper about linear analysis.
 * This snapshot was made so that I could have a constant test case
 * for the results section which I could modify without issue.
 *
 * Andrew Lamb, 11/4/2002
 **/
...
class EqualizerInnerPipeline extends Pipeline 
{
    ...
    {
        add(new LowPassFilter(rate, freq, 64));
        add(new FloatDup());
    }
}
...
class EqualizerSplitJoin extends SplitJoin {
	...
	setSplitter(DUPLICATE());
        add(new LowPassFilter(rate, high, 64));
        add(new EqualizerInnerSplitJoin(rate, low, high, bands));
        add(new LowPassFilter(rate, low, 64));
...
class Compressor extends Filter {
	...
	M = decimation;
	input  = new Channel(Float.TYPE, decimation, decimation);
	output = new Channel(Float.TYPE, 1);
    }
    public void work() {
 ...
 * Class LowPassFilter
 *
 * Implements a Low Pass FIR Filter using a Hamming windowed
 * sinc function.
 */
...
public class FMRadioApp extends StreamIt
{
    static public void main(String[] t)
    {
        new FMRadioApp().run(t);
    }
