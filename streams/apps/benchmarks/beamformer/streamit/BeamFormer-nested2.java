/*
 * This version is more finely nested than Beamform-nested1.  It
 * breaks the top splitjoin into two pieces to try to have a
 * hand-partitioning of the given parameters.  Though this looked like
 * it might have worked well, the performance was dismal (maybe due to
 * routing?).  So it's in CVS just in case someone wants to try again.
 */
...
public class BeamFormer extends StreamIt
{
	...
	final int GENERATE_BLOCKING1     = 8;
	final int GENERATE_BLOCKING2     = 12;
	// how many streams per hierarchical splitjoin in the detect phase
	final int DETECT_BLOCKING       = 4;

	final int numChannels           = 48;//48;
	final int numSamples            = 64;//4096;
	final int numBeams              = 16;//16;
	...
	add(new SplitJoin() {
		public void init() {
		    int i;
		    setSplitter(NULL());
		    for(i=0; i<numChannels; i+=GENERATE_BLOCKING1) {
			this.add (new GenerateSJWrapper1(i,
							numSamples,
							targetBeam,
							targetSample,
							numCoarseFilterTaps,
		    ...
		    setJoiner(ROUND_ROBIN(2*GENERATE_BLOCKING1));
		}
	    });
	...
	add(new SplitJoin() {
		public void init() {
		    int i;
		    setSplitter(ROUND_ROBIN(2*GENERATE_BLOCKING2));
		    for(i=0; i<numChannels; i+=GENERATE_BLOCKING2) {
			this.add (new GenerateSJWrapper2(i,
							numSamples,
							targetBeam,
							targetSample,
							numCoarseFilterTaps,
		    ...
		    setJoiner(ROUND_ROBIN(2*GENERATE_BLOCKING2));
		}
	    });
...
class GenerateSJWrapper1 extends Pipeline {
    public GenerateSJWrapper1(int i,
			     int numSamples,
			     int targetBeam,
			     int targetSample,
...
class GenerateSJWrapper2 extends Pipeline {
    public GenerateSJWrapper2(int i,
			     int numSamples,
			     int targetBeam,
			     int targetSample,
			     int numCoarseFilterTaps,
...
class GenerateSJ1 extends SplitJoin {
    public GenerateSJ1(int i,
			     int numSamples,
			     int targetBeam,
			     int targetSample,
			     int numCoarseFilterTaps,
...
class GenerateSJ2 extends SplitJoin {
    public GenerateSJ2(int i,
			     int numSamples,
			     int targetBeam,
			     int targetSample,
			     int numCoarseFilterTaps,
