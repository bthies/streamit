...
public class SimpleScheduler {
    ...
    public SimpleScheduler(Partitioner partitioner, RawChip rawChip) {
        ...
        // spSched = new SpaceTimeSchedule(rawChip.getYSize(),
        // rawChip.getXSize());
    }
    ...
    public boolean scheduledBefore(Trace t1, Trace t2) {
        ...
        assert schedule.contains(t1) && schedule.contains(t2) && t1 != t2;
    ...
    public void schedule() {
        initSchedule = DataFlowOrder.getTraversal(partitioner.topTraces);
    ...
    private void scheduleCommunication() {
        ...
        Arrays.sort(tempArray, new CompareTraceCommunication());
    ...
    private boolean canScheduleTrace(Trace trace) {
        // check to make sure all srcs are in the same "state" either scheduled
        // or not
        ...
        // check to make sure all dests are in the same "state" either scheduled
        // or not
    ...
    private void removePredefined(LinkedList sortedTraces) {
        for (int i = 0; i < partitioner.io.length; i++) {
            /*
             * Some old code, ignore HashMap layout =
             * canLayoutTrace(partitioner.io[i]); assert layout != null :
             * "Cannot find tile for predefined filter"; //"assign" the
             * predefined to a tile... FilterTraceNode node =
             * partitioner.io[i].getHead().getNextFilter(); assert
             * node.isPredefined() && layout.containsKey(node) &&
             * layout.get(node) != null; //assign RawTile tile =
             * (RawTile)layout.get(node);
             * ((FilterTraceNode)node).setXY(tile.getX(), tile.getY());
             * System.out.println("Assigning " + node + " to " + tile); //record
             * that we used this port for a file reader or writer... if
             * (((FilterTraceNode)node).isFileInput()) { assert
             * tile.hasIODevice() && !readsFile[tile.getTileNumber()];
             * readsFile[tile.getTileNumber()] = true;
             *  } else if (((FilterTraceNode)node).isFileOutput()) { assert
             * tile.hasIODevice() && !writesFile[tile.getTileNumber()];
    ...
    private boolean scheduleSmallerTrace(LinkedList sortedTraces, Trace bigTrace) {
        ...
        while (it.hasNext()) {
            ...
            if ((currentTime + partitioner.getTraceBNWork(trace)) <= finishBefore) {
    ...
    private HashMap canLayoutTrace(Trace trace) {
        ...
        // build the list of tiles, the order of the list will be the order
        // we inspect tiles for availability, so randomize this to prevent
        // tile from being over-used
        int startingTile = rand.nextInt(rawChip.getTotalTiles());

        for (int i = 0; i < rawChip.getTotalTiles(); i++) {
            tiles.add(rawChip.getTile(startingTile++
                                      % rawChip.getTotalTiles()));
        }
        ...
        if (KjcOptions.forceplacement
            ...
            Trace upstream = ((Edge) trace.getHead().getSourceSet().iterator()
                              .next()).getSrc().getParent();
            if (schedule.contains(upstream)
                && upstream.getTail().getDestSet().size() == 1) {
                RawTile tile = rawChip.getTile(upstream.getTail()
                                               .getPrevFilter().getX(), upstream.getTail()
                                               .getPrevFilter().getY());
                System.out.println("** We want to place this trace at " + tile
                                   + " but " + tiles.contains(tile));
                for (int i = 0; i < tiles.size(); i++)
                    System.out.println("  " + tiles.get(i));
                if (tiles.contains(tile)) {
                    System.out
                        .println("Trying to layout of entire trace starting at "
                                 + tile);
        ...
        // if we want to, try to force a single output trace to the dest's tile
        // as long as the
        // downstream tile has one input
        if (KjcOptions.forceplacement
            && trace.getTail().getDestSet().size() == 1) {
            Trace downstream = ((Edge) trace.getTail().getDestSet().iterator()
                                .next()).getDest().getParent();
            if (schedule.contains(downstream)) {
                RawTile tile = rawChip.getTile(downstream.getTail()
                                               .getPrevFilter().getX(), downstream.getTail()
                                               .getPrevFilter().getY());
                if (tiles.contains(tile)
                    && downstream.getHead().getSourceSet().size() == 1) {
                    HashMap layout = new HashMap();
                    // try to place on the tile ignoring the tile avail
                    if (getLayout(trace.getHead().getNextFilter(), tile,
                                  layout, true))
        ...
        while (sources.hasNext()) {
            ...
            if (!(current.getTail().getDestSet().size() == 1))
                continue;
            // now if the source is already scheduled or if it is io try its
            // tile
            if (schedule.contains(current)) {
                // get the endpoint of the trace
                RawTile tile = rawChip.getTile(current.getTail()
                ...
                /*
                 * //if the source outputttracenode has more than one output
                 * don't do this if (!current.getTail().oneOutput()) continue;
                 */

                HashMap layout = new HashMap();
                // System.out.println(" (for " + trace + " trying source " +
                // tile + " at " + currentTime + ")");
                // if successful, return layout
                if (getLayout(trace.getHead().getNextFilter(), tile, layout,
                              false))
                    return layout;
                else
                    // we tried this tile, remove it...
                    tiles.remove(tile);
            }
        }

        /*
         * Iterator dests = trace.getTail().getDestSet().iterator(); while
         * (dests.hasNext()) { Trace current =
         * ((Edge)dests.next()).getDest().getParent(); if
         * (schedule.contains(current)) { //get the endpoint of the trace
         * RawTile tile =
         * rawChip.getTile(current.getHead().getNextFilter().getX(),
         * current.getHead().getNextFilter().getY()); //we already tried this
         * tile previously if (!tiles.contains(tile)) continue; HashMap layout =
         * new HashMap(); //System.out.println(" (for " + trace + " trying dest " +
         * tile + " at " + currentTime + ")"); //if successful, return layout if
         * (getLayout(trace.getHead().getNextFilter(), tile, layout, false))
         * return layout; else //we tried this tile, remove it...
         * tiles.remove(tile); } }
         */
        ...
        while (tilesIt.hasNext()) {
            ...
            HashMap layout = new HashMap();
            System.out.println("  ************ trying " + tile
                               + " *********** for " + trace);
            // System.out.println(" (for " + trace + " trying " + tile + " at "
            // + currentTime + ")");
            // if successful, return layout
            if (getLayout(trace.getHead().getNextFilter(), tile, layout, false))
                return layout;
        }
    ...
    private boolean getLayout(FilterTraceNode filter, RawTile tile,
                              HashMap layout, boolean ignoreAvail) {
        ...
        if ((filter.getNext().isOutputTrace() || filter.getPrevious()
             .isInputTrace())
            && !tile.hasIODevice()) {
        ...
        // right now we are assuming that all traces reading and writes files
        // have
        ...
        if (filter.getNext().isOutputTrace()) {
            ...
            if (out.hasFileOutput()) {
                ...
                if (writesFile[tile.getTileNumber()]) {
                    // System.out.println(" (Tile already used for file
                    // writer)");
                    return false;
                }
        ...
        /*
         * //check file readers/writers, they must be //on border and each tile
         * can have one of each if (filter.isFileInput() && !(tile.hasIODevice() &&
         * !readsFile[tile.getTileNumber()])) { //System.out.println(" (Failed
         * file reader)"); return false; } if (filter.isFileOutput() &&
         * !(tile.hasIODevice() && !writesFile[tile.getTileNumber()])) {
         * //System.out.println(" (Failed file writer)"); return false; }
         */
