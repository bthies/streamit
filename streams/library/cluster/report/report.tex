...
The time interval for considering a reconfiguration, which is
currently set at 40 seconds could be adjusted dynamically.
The difference between the current and the new mapping
could be calculated differently and the heuristic for
deciding when to reconfigure be adjusted accordingly.
Lastly, our partitioner which is greedy could be improved
to use some heuristic or a search to produce better mapping. 

\section{Conclusion}
 
In this work we have proposed and implemented a new load balancing
algorithm for Streamit, that can benefit any stream application
running on Streamit clustered machines. We used static and run-time
gathered statistics to dynamically adjust load balance for a
non-homogeneous cluster, and showed that this would significantly
improve the performance of a StreamIt application. It also allows us
to adjust load balance if a computationally intensive background
process is executing or started on one of the cluster machines.
\newpage
