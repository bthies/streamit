...
public class EdgeView extends AbstractCellView {
	...
	protected Shape beginShape, endShape, lineShape;

	/** Shared-path tune-up. */
	protected transient GeneralPath sharedPath = null;
     ...
     */
    final Rectangle getLabelBounds() {
	...
	public class EdgeHandle implements CellHandle, Serializable {
		...
		public void mouseDragged(MouseEvent event) {
			...
			if (label) {
				...
				if (r != null) {
				    ...
				    int xunit = 1;
				    if (r.width != 0)
					xunit = GraphConstants.PERCENT / r.width;
				    int yunit = 1;
				    if (r.height != 0)
					yunit = GraphConstants.PERCENT / r.height;
		...
		protected void invalidate() {
			...
			EdgeRenderer er = (EdgeRenderer) EdgeView.renderer;
			for (int i = 0; i < r.length; i++) {
