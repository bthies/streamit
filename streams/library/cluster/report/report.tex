...
\title{\ \\\vspace{40pt}Dynamic Load Balancing for Executing\\ Stream Programs on a Cluster\\ \ \\}
...
balancing to the approach that assumes that equal CPU resources 
are available an all cluster machines.
...
and compiler that facilitates modularity, programming productivity, and 
...
the machines fails or is disconnected from the network. We will modify the 
...
Load balancing algorithms have been studied in various contexts like CPU usage
in parallel processing and route selection in communication networks. For our
study, we only consider CPU utilization for load balancing and assume the
network bandwidth is abundant and that memory usage is not a concern. Static
and dynamic load balancing have been studied in \cite{ghosh}, \cite{hamdi}, and \cite{zhang}. \cite{zhang} uses simulation
approach to compare the performance of dynamic and static load balancing in
heterogeneous distributed systems. \cite{ghosh} studies dynamic load balancing in the
context of graph theory and solves the problem of Incremental Weight Migration
on an arbitrary graph. They propose a randomized algorithm that results in
optimal convergence towards the equal weighted graph. \cite{hamdi} introduces a dynamic
load balancing scheme for parallel image processing applications, where a
central manager is used to partition a given workload (image) into a set of
several smaller sub-images, distribute them to all components involved, and
collect the results. In their computing environment, bandwidth of the network
is limited, and they try to optimize the algorithm so that communication cost
involved in transferring the data is not greater than the cost of
computing the
data locally. Also, their load balancing algorithm is very specific to
the image
processing application. Like \cite{zhang}, our dynamic load balancing algorithm uses a
centralized manager and the graph theory approach to compute the
repartitioning
of the workload assigned to each cluster machine. Our dynamic load balancing
algorithm implementation is specific to Streamit, and any 3rd party
application, that runs on Streamit can benefit from the load balancing.
...
In StreamIt each program can be represented as a garph with nodes
and edges. Each node represents a computation element and each edge
represents a FIFO data channel. For an example graph see 
Figure~\ref{part1}. To perform file I/O the first node can read
data from a file and write it to its output channel, the last
node can read data from its input channel and write it to a file.
...
\begin{center}
   ...
   \caption{Some partitions not considered by the existing partitioner}
   \label{part2}
\end{center}
\end{minipage}
\end{center}
...
machine and informs it that a machine is available for stream processing. 
The CCP then gives instructions to the NodeServer and periodically 
checks that the machine is alive.

Each time that a new cluster machine connects to the CCP or an connected
machine becomes unavailable the assignment of threads to machines has to be 
changed.

\begin{list}{}
\item 1. The CCP sends a STOP\_ALL\_THREADS message to all connected machines.
\item 2. The CCP reads a mapping for the threads to a given number of machines from a file (a set of files has to be pre-generated statically).
...
In order to support the restart of a stream application, each computation 
...
StreamIt program is $util_i$ and CPU has $idle_i$ idle cycyles
...
machine $i$ by using a following formula:
...
Note that the algorithm resembles a breadth first search and that the 
last node becomes eligible only after the both previous nodes have been 
...
thresholds are proportional to the $R_i$ values for the machines.
...
average values of all statistics gathered during the last 40 seconds.
...
3. If ( More Than 4 Percent Differece )\\
...
different machine in the new mapping is more than 4 percent of the
sum of all work estimates, the CCP initiates a reconfiguration. 
During a reconfiguration the CCP sends 
...
We study the performance of our dynamic load balancing for different 
cluster configurations. Our cluster consists of dual processor machines
with processor speeds of 840MHz, 1GHz, 2.2GHz and 3.6GHz.
The network has a capacity of 1 gigabit/second and it is fully switched. 
We use a MPEG-2 Decoder implementation in StreamIt for evaluating our load 
balancing algorithm. We chose this benchmark because it can be
run for an extended amount of time alowing us to see the effect of 
dynamic load balancing over time. We measure the performance by inserting 
a timer in the last thread (the thread that writes data to a file), the timer 
measures the time it takes to gather data corresponding to a single frame. 
This timer reflects the steady state performance of the StreamIt program. 
...
Figure~\ref{fig1} and Figure~\ref{fig2} show the performance of MPEG2 
...
reconfiguration is higher than with the 8~percent threshold. 
We observe that our dynamic load balancing has aproximately same performance
...
\begin{center}
   ...
   \caption{MPEG2 decoder performance on a 3~machine cluster (all~1GHz) with a background process started on one of the machines after 2 minutes.}
   \label{fig6}
\end{center}
\end{minipage}
\end{center}
...
The Figure~\ref{fig3} shows the performance on a homogeneous 3 machine
...
is about the same. The Figure~\ref{fig6} shows the performance on
a homogeneous 3 machine cluster with a background process that starts
on one of the machines two minutes after the start of computation.
We see that without dynamicl load balancing the performance degrades
...
\begin{center}
   ...
   \caption{MPEG2~decoder~performance~on~a 4~machine~cluster~(840MHz~-~3.6GHz) with a background process after frame 9.}
   \label{fig5}
\end{center}
\end{minipage}
\end{center}
...
Figure~\ref{fig4} and Figure~\ref{fig5} show how our dynaimc load 
