...
public class ShowQueueValuesAction extends Action {
	...
	public ShowQueueValuesAction() {
		super(ActionMessages.getString("ShowQueueValues.title")); //$NON-NLS-1$
		setDescription(ActionMessages.getString("ShowQueueValues.toolTipText")); //$NON-NLS-1$
		
		ImageRegistry ir = StreamItDebuggerPlugin.getDefault().getImageRegistry();
		setImageDescriptor(ir.getDescriptor(IStreamItDebuggerPluginConstants.SHOW_CHANNEL));
		setDisabledImageDescriptor(ir.getDescriptor(IStreamItDebuggerPluginConstants.FADED_SHOW_CHANNEL));
	}
	...
	protected void doActionPerformed(final Channel c) {
		...
		try {
			...
			DebugUIPlugin.errorDialog(activeShell, ActionMessages.getString("ShowQueueValues.errorDialogTitle"),ActionMessages.getString("ShowQueueValues.errorDialogMessage"), exception);	//$NON-NLS-2$ //$NON-NLS-1$
			fInputDialog= null;
			return;
		}
		
		fInputDialog= new ShowQueueValuesDialog(activeShell, ActionMessages.getString("ShowQueueValuesSet_Variable_Value_1"), ActionMessages.getString("ShowQueueValuesEnter_a_new_value_for_2"), value, new IInputValidator() { //$NON-NLS-1$ //$NON-NLS-2$
			/**
