...
public class LEETFrequencyReplacer extends FrequencyReplacer{
	...
	/* make the fields to hold the transform of the weights. Note that the
	   ...
	   generated by FFTW. */
	JVariableDefinition weightField = makeWeightField(FILTER_WEIGHTS_PREFIX + "_0");
	...
	JVariableDefinition partialField   = makeWeightField(PARTIAL_BUFFER_PREFIX);
	newFields = appendFieldDeclaration(newFields, partialField);

	/* make the two buffer fields (input and output). **/
	...
	JVariableDefinition outputBufferField = makeWeightField(OUTPUT_BUFFER_NAME);
	newFields = appendFieldDeclaration(newFields, outputBufferField);
	

	
	/* make a new init function */
	JMethodDeclaration freqInit = makeNewInit(linearRep,
						  weightField, partialField,
						  inputBufferField, outputBufferField,
						  filterSize, x);
    ...
    public JMethodDeclaration makeNewInit(LinearFilterRepresentation linearRep,
					  ...
					  JVariableDefinition outputBufferField,
	...
	/** add in statements to allocate space for the weight field, partial results
	    field, input buffer field, and output buffer field.
 	...
 	body.addStatement(makeFieldAllocation(outputBufferField.getIdent(), filterSize,
	...
	/* now, copy the data from the input tape into the input buffer, element by element */
