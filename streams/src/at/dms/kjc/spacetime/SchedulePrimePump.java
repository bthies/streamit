    ...
    private static void incrementUpstream(Trace trace, SimpleScheduler scheduler) {
        ...
        while (sources.hasNext()) {
            ...
            if (scheduler.scheduledBefore(source, trace)) {
                ...
                // "Unsupported schedule created (source: " + source.toString()
                // + ", dest: " + trace.toString();
    ...
    private static void checkGraph(SimpleScheduler schedule) {
        ...
        while (traces.hasNext()) {
            ...
            if (sources.hasNext()) {
                ...
                assert diff == 0 || diff == 1 : "Illegal primepump schedule "
                    + diff;
                while (sources.hasNext()) {
                    src = ((Edge) sources.next()).getSrc().getParent();
                    // assert all one off
                    // if (src.getPrimePump() - trace.getPrimePump() != diff)
                    // System.out.println("Illegal primepump schedule2 " +
                    // trace);
                    assert Math.abs(src.getPrimePump() - trace.getPrimePump()) == diff : "Illegal primepump schedule";
                }
            }
            ...
            if (dests.hasNext()) {
                ...
                while (dests.hasNext()) {
                    ...
                    // System.out.println("Illegal primepump schedule3 " +
                    // trace);
                    assert Math.abs(trace.getPrimePump() - dst.getPrimePump()) == diff : "Illegal primepump schedule "
                        + diff
                        + " != "
                        + Math.abs(trace.getPrimePump()
                                   - dst.getPrimePump());
                }
            }
