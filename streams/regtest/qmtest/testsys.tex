...
\subsubsection{examine-results.py}
Executable, invoked from run-reg-tests.py to produce a brief report based on
the current QMTest results file, and differences from the ``latest''
QMTest results file if one exists. The first parameter is the path to
the current QMTest results file, the second parameter is the path to
the latest QMTeat resultle file.
...
{\tt RegtestItem}, and 
{\tt listing.html}.
Presumably these files are used to show the latest QMtest results on the 
RT ticket tracking system.  The files are not very readable on their own.
...
nightly regression test using the command:
