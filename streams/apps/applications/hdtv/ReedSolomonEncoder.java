...
public class ReedSolomonEncoder extends Filter {
	...
	int K = 187; // total data bytes (note that N - K = T = number of ECC bytes) 
	...
	int i;
	char codeword[] = new char[255]; // space for the encoded message data
...
//    }
  ...
  void decode_data(char ddata[], int nbytes)
  {
    int i, j, sum;
    for (j = 0; j < NPAR;  j++) {
      sum	= 0;
